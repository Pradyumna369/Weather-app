...
const WeatherCard = ({temperature, setCityName, cityName, humidity, windSpeed, loading, error, icon, desc, setUnit}) => {
    ...
    <div className='weather-app'>
        ...
        <div className='mt-10 items-center'>
            ...
            <span className="select-none me-3 text-sm font-medium text-heading">Metric</span>
        ...
        <form 
          className='flex city-row justify-between'
          onSubmit={(e) => {
            e.preventDefault();
            setCityName(city);
          }}>
        ...
        <button type="submit" className='bg-sky-500 hover:bg-sky-700 rounded-lg w-40 h-10 cursor-pointer'>Get Weather</button>
        </form>
        ...
        <div className='weather-card rounded-lg'>
          ...
          <div className='flex w-full text-gray-400 items-center px-4 justify-between h-10'>
              <div role="status" aria-live="polite" className='flex w-1/2 items-center'>
                {loading && <><div><img src={Loading_icon} className='w-10 h-10'/>
              </div>
                <div>Loading...</div></>}</div>
            <div className='flex'>
              <div>Humidity: </div>
